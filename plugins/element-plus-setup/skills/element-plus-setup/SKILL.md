...
## Step 8 — Ask about base components

Ask the user:

> Do you want me to create a set of reusable base components that wrap Element Plus?
...
```
Base components to create under src/components/base/:

[ ] BaseButton    — el-button wrapper with project-wide variant presets
[ ] BaseInput     — el-input with inline label, error message, and helper text
[ ] BaseSelect    — el-select with typed { label, value } options array prop
[ ] BaseDialog    — el-dialog with header/footer slots and confirm/cancel emits
[ ] BaseDrawer    — el-drawer with consistent width, title, and footer slot
[ ] BaseTable     — el-table with loading state, empty state, and built-in pagination
[ ] BasePagination — el-pagination with sensible layout and page-size defaults
[ ] BaseCard      — el-card with optional title prop and header/default/footer slots
[ ] BaseTag       — el-tag mapping a semantic type prop to Element Plus types
[ ] BaseAlert     — el-alert with auto-close timer and dismissible defaults
[ ] BaseEmpty     — el-empty with a customisable description and action slot
[ ] BaseSkeleton  — el-skeleton with configurable rows for loading placeholders
[ ] BaseUpload    — el-upload with drag-and-drop, accept, and max-size validation
[ ] BaseFormItem  — el-form-item with label, tooltip icon, and required indicator
```

All components go in `src/components/base/`. Each file name matches the component name exactly (e.g., `BaseButton.vue`).
...
<style scoped>
.base-input__label { display: block; font-size: 13px; font-weight: 500; margin-bottom: 4px; }
.base-input__required { color: var(--el-color-danger); margin-left: 2px; }
.base-input__error { font-size: 12px; color: var(--el-color-danger); margin-top: 4px; }
.base-input__hint { font-size: 12px; color: var(--el-text-color-secondary); margin-top: 4px; }
</style>
```
...
<style scoped>
.base-table__pagination { margin-top: 16px; justify-content: flex-end; }
</style>
```
    ...
    <template #label>
      <span class="base-form-item__label">
        ...
        <el-tooltip v-if="tooltip" :content="tooltip" placement="top">
          <el-icon class="base-form-item__icon"><QuestionFilled /></el-icon>
        </el-tooltip>
      </span>
    </template>
    <slot />
...
<style scoped>
.base-form-item__label { display: flex; align-items: center; gap: 4px; }
.base-form-item__icon { cursor: help; color: var(--el-text-color-secondary); }
</style>
```
...
- [ ] Selected base components created under `src/components/base/` (if requested)
