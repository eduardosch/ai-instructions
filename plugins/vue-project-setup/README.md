# `vue-project-setup` Claude Skill

Scaffolds a new Vue 3 project with an opinionated stack: TypeScript, JSX, Vue Router, Pinia, Playwright, ESLint, Prettier, and Vue DevTools — then automatically installs the `commit-message`, `versioning`, `vue-style-guide`, and `vue-ts-style-guide`, and `pinia-style-guide` plugins so the project is ready to go from the first commit.

## Installation

Launch Claude Code first:

```bash
claude
```

Then from within Claude Code, add the marketplace and install the skill:

```bash
/plugin marketplace add eduardosch/ai-instructions

/plugin install vue-project-setup@eduardosch-marketplace
```

## Uninstalling

```bash
/plugin uninstall vue-project-setup

/plugin marketplace remove eduardosch-marketplace
```

## Usage

```
/vue-project-setup
```
