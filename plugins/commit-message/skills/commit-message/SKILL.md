...
7. Determine emoji usage:
   - Add emojis using the mapping table below, placing the emoji before the type
...
12. Check for the presence of breaking changes
...
13. New modules must be a major commit
   - Example with new module: feat!(scope): add a new module to the project
   - Example with new message: feat!: commit message
...
```text
<emoji><type>/<task-description>
```

Where:

- `<emoji>`: Is the emoji referring to the type
...
- Always add emojis on the commit messages
